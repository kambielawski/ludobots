...
class Quadruped2X:
    def __init__(self, solnId, dir='.'):
        self.solnId = solnId
        self.dir = dir
        self.sourceBodyFile = "robots/body_quadruped2x.urdf"
        self.bodyFile = f"{self.dir}/body_quadruped2x_{self.solnId}.urdf"
    ...
    def Generate_Body(self, start_x, start_y, start_z):
        start_z += 1 # Increase the body size
        ...
        pyrosim.Send_Cube(name="FrontLeg", pos=[0,1,0], size=[0.2,2,0.2])
        pyrosim.Send_Cube(name="LeftLeg", pos=[-1,0,0], size=[2,0.2,0.2])
        pyrosim.Send_Cube(name="RightLeg", pos=[1,0,0], size=[2,0.2,0.2])
        pyrosim.Send_Cube(name="BackLeg", pos=[0,-1,0], size=[0.2,2,0.2])

        pyrosim.Send_Cube(name="BackLower", pos=[0,0,-1], size=[0.2,0.2,2])
        pyrosim.Send_Cube(name="FrontLower", pos=[0,0,-1], size=[0.2,0.2,2])
        pyrosim.Send_Cube(name="LeftLower", pos=[0,0,-1], size=[0.2,0.2,2])
        pyrosim.Send_Cube(name="RightLower", pos=[0,0,-1], size=[0.2,0.2,2])
